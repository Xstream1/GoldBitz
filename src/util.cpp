...
const char * const BITCOIN_CONF_FILENAME = "goldkash.conf";
const char * const BITCOIN_PID_FILENAME = "goldkashd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "goldkash" is a composite category enabling all GoldKash-related debug output
            if(ptrCategory->count(string("goldkash"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GoldKashCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GoldKashCore
    // Mac: ~/Library/Application Support/GoldKashCore
    // Unix: ~/.goldkashcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GoldKashCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GoldKashCore";
#else
    // Unix
    return pathRet / ".goldkashcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty goldkash.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override goldkash.conf

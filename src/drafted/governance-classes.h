 ...
 *
 *   // network
 *   CGoldKashNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.goldkash.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CGoldKashNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CGoldKashNetwork(UniValue objIn)
    {
...
// // can be under: GoldKashNetwork
// //   -- signature requirements : Key1(User)
// class CGoldKashNetworkVariable : public CGovernanceObject
// {
// private:
